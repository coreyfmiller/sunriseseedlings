...
export function PlantCard({ plant }: { plant: PlantData }) {
  return (
    <article
      className={`group relative flex flex-col overflow-hidden rounded-2xl border-4 ${
        borderColors[plant.borderColor] ?? "border-border"
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          <button
            className="flex items-center gap-1.5 rounded-2xl bg-sun-yellow px-4 py-2 text-sm font-bold text-foreground shadow-sm transition-transform hover:scale-105 active:scale-95"
